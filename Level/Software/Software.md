...
- 层次化技巧：如何处理各层次面临的问题
    1. 解决组件物理循环依赖的方法
        ...
        # 作用：消除对大型子系统的依赖。【本质：通过名称上使用（不透明指针）消除大型子系统的依赖，同时带来了对象析构的问题。回调可以解决这个问题】
        # 
        # 1.避免回调必要使用的例子
        # 2.虚函数是一种回调机制。名称上的使用可以带来类型上的安全。
        #   【面向对象基本类型的结构设计要点】
        #    抽象基类声明了功能却没有提供实现【viritual机制】
        #    声明了作用对象是点却没有提供点的组成细节【名称上使用】
        #    同时提供了类型安全
        # 3. 类型安全的健壮性代码写法 
        ```
        ...
        ```
        # 【工程层次化开发流程】
        #  1.【更好的工程灵活性】：将需求翻译成计算机语言：输入-黑盒子-输出。做好功能组件划分。
        #  2.【更好的编译/链接性能】：识别组件中的接口依赖和实现依赖，通过升级/降级去除物理循环依赖。
        #  3.【更好的封装性】：识别同层次组件间的协作关系，使用不透明指针让组件的功能更加内聚，专一。
        #  4.【更好的运行时性能】：识别组件间的名字上循环依赖，权衡是否通过哑数据优化。
        #  5.【更好的可维护性】：识别组件间高CCD依赖节点，进行冗余优化。
        ```
